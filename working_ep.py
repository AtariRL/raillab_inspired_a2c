...
class Memory(object):
    def __init__(self, states, rewards, mask, next_state, log_probs, entropies):
        self.states = states
...
def sample(observation):
    states, rewards, masks, log_probs, entropies = [], [], [], [], []
    for step in range(5):
    ...
    return Memory(states, rewards, masks, next_observation, log_probs, entropies)
...
if __name__ == '__main__':
    ...
    for update in range(num_updates):
        ...
        actor_loss, critic_loss, entropy_loss = update_gradient(memory)

        #if len(replay_buffer) > replay_memory_max_length:
        #    del replay_buffer[:4]
